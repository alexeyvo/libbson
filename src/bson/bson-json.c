                        ...
                        size_t               vlen) /* IN */
{
   ...
   if (rs == BSON_JSON_REGULAR) {
      bson_append_utf8 (BSON_JSON_STACK_BSON_CHILD, key, (int)len, (const char *)val, (int)vlen);
   } else if (rs == BSON_JSON_IN_BSON_TYPE || rs ==
              BSON_JSON_IN_BSON_TYPE_TIMESTAMP_VALUES) {
...
_bson_json_read_end_array (void *_ctx) /* IN */
{
   ...
   BSON_JSON_STACK_POP_ARRAY (bson_append_array_end (BSON_JSON_STACK_BSON_PARENT,
                                                     BSON_JSON_STACK_BSON_CHILD));
